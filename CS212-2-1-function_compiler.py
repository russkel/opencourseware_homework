...
def compile_formula(formula, verbose=False):
    ...
    first_letters = list(first_digits(re.split('([A-Z]+)', formula)))
    if len(first_letters) > 0:
        check = [x + " != 0" for x in first_letters]
        check = " and " + " and ".join(check)
    else:
        check = ''
    body = ''.join(tokens)
    print check
    f = 'lambda %s: (%s)%s' % (parms, body, check)
    if verbose: print f
    return eval(f), letters

def first_digits(words):
    for word in words:
