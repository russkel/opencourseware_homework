...
def poly(coefs):
    ...
    parts = []
    for (n, co) in enumerate(coefs):
        if co == 0: continue # drop 0 * x**n
        
        l, r = [str(co), str(xto(n))]
        ...
        parts.append(" * ".join(filter(None, [l,r])))
            
    formula = " + ".join(reversed(parts))
    if formula == '': formula = "0" 
    #print formula, coefs, parts
    f = eval("lambda x: %s" % formula)
...
def add(p1, p2):
    ...
    return poly(tuple(a+b for a, b in itertools.izip_longest(p1.coefs, p2.coefs, fillvalue=0)))
...
def mul(p1, p2):
    ...
    c = [0] * (len(p1.coefs) + len(p2.coefs) - 1)
...
def deriv(p):
    ...
    return poly(tuple(c*n for (n, c) in enumerate(p.coefs) if n > 0))

def integral(p, C=0):
    "Return the integral of a function p (with respect to its argument)."
#        assert integral(poly((20, 60))).coefs == (0, 20, 30)

#    print (C,) + tuple(c/(n+1) for (n, c) in enumerate(p.coefs))
    return poly((C,) + tuple(c/(n+1) for (n, c) in enumerate(p.coefs)))
