...
def anagrams(phrase, shortest=2):
    ...
    letters = "".join([l.upper() for l in phrase if l in string.letters])
    print "Letters:", letters
    
    anagram_set = find_anagrams(set(), letters, [], shortest)
    print anagram_set
    return anagram_set

def find_anagrams(anagram_set, letters, anag, shortest):
    words = find_words(letters)
    #print anag, letters, words
    #no letter left, therefore we have found everything
    if letters == '' and not words:
        anagram_set.add(" ".join(sorted(anag)))
        #print anagram_set
        return
    if words == []: return #no more words  
    
    for word in words:
        if len(word) >= shortest:
            find_anagrams(anagram_set, removed(letters, word), anag + [word], shortest)
