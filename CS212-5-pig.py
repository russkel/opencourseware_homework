...
def strategy_d(state):
    (p, me, you, pending, double) = state
    probab = Pwin3(me,you,pending)
    if double == 'double':
        return 'decline' if probab < 0.3 else 'accept'    
    if me + pending >= goal:
        if 'double' in pig_actions_d(state):
            return 'double'
        else:
            return 'hold'
    if 'double' in pig_actions_d(state) and probab > 0.9:
        return 'double'
    if 'hold' in pig_actions_d(state) and 1 - Pwin3(you,me+pending,0) == probab:
        return 'hold'
    return 'roll'
