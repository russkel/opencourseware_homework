...
def subway(**lines):
    ...
    lines = dict((line, stations.split(' ')) for line, stations in lines.items())
    stations = set(s for l in lines.values() for s in l)
        
    return dict((s, dict(get_neighbours(s, lines))) for s in stations)
    
def get_neighbours(station, lines):
    neighbours = []
    for line, stations in lines.items():
        if station in stations:
            idx = stations.index(station)
            if idx == 0:
                neighbours += [(stations[idx+1], line)]
            elif idx == len(stations)-1:
                neighbours += [(stations[idx-1], line)]
            else:
                neighbours += [(stations[idx-1], line), (stations[idx+1], line)]
...
def ride(here, there, system=boston):
    ...
    def subway_goal(state): return state == there
    def subway_successors(state): return system[state]
    return shortest_path_search(here, subway_successors, subway_goal)

def longest_ride(system):
    """"Return the longest possible 'shortest path' 
    ride between any two stops in the system."""
    return max([ride(here, there, system)
            for here, there in permutations(system.keys(), 2)], key=len)
